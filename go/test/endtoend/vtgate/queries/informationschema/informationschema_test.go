...
func TestDbNameOverride(t *testing.T) {
	...
	assert.Equal(t, cluster.DbPrefix+"ks", qr.Rows[0][0].ToString())
}
...
func TestInformationSchemaQuery(t *testing.T) {
	...
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = 'ks'", cluster.DbPrefix+"ks")
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = '"+cluster.DbPrefix+"ks'", cluster.DbPrefix+"ks")
	...
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = '"+cluster.DbPrefix+"ks' and table_name = 't1'", cluster.DbPrefix+"ks")
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = 'ks' and table_name = 't1'", cluster.DbPrefix+"ks")
}
