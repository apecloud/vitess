...
func TestSetDefaultReadWriteSeparationStrategy(t *testing.T) {
	type args struct {
		strategy string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.wantErr(t, SetDefaultReadWriteSeparationStrategy(tt.args.strategy), fmt.Sprintf("SetDefaultReadWriteSeparationStrategy(%v)", tt.args.strategy))
		})
	}
}
