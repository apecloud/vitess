...
const (
	...
	// DirectiveNodeType specifies the node type for the query. possible values are: PRIMARY/REPLICA/RDONLY
	DirectiveNodeType = "NODE_TYPE"
)
...
func GetNodeType(stmt Statement) tabletpb.TabletType {
	...
	directives := comments.Directives()
	strv, isSet := directives.GetString(DirectiveNodeType, "PRIMARY")
	if !isSet {
		return tabletpb.TabletType_UNKNOWN
	}
