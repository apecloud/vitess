...
func (m *Session) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ReadWriteSeparationStrategy) > 0 {
		i -= len(m.ReadWriteSeparationStrategy)
		copy(dAtA[i:], m.ReadWriteSeparationStrategy)
		i = encodeVarint(dAtA, i, uint64(len(m.ReadWriteSeparationStrategy)))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0xd2
...
func (m *Session) SizeVT() (n int) {
	...
	l = len(m.ReadWriteSeparationStrategy)
	if l > 0 {
		n += 2 + l + sov(uint64(l))
	}
	n += len(m.unknownFields)
...
func (m *Session) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 26:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ReadWriteSeparationStrategy", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ReadWriteSeparationStrategy = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
