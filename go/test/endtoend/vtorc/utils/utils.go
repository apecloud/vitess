...
func cleanAndStartVttablet(t *testing.T, clusterInfo *VTOrcClusterInfo, vttablet *cluster.Vttablet) {
	...
	_, err = RunSQL(t, "DROP DATABASE IF EXISTS "+cluster.DbPrefix+"ks", vttablet, "")
	require.NoError(t, err)
...
func CheckReplication(t *testing.T, clusterInfo *VTOrcClusterInfo, primary *cluster.Vttablet, replicas []*cluster.Vttablet, timeToWait time.Duration) {
	...
	sqlSchema := `
		create table if not exists ` + cluster.DbPrefix + `ks.vt_insert_test (
		id bigint,
		msg varchar(64),
		primary key (id)
		) Engine=InnoDB
	...
	for {
		select {
		case <-timeout:
			t.Fatal("timedout waiting for keyspace " + cluster.DbPrefix + "ks to be created by schema engine")
			return
		default:
...
func confirmReplication(t *testing.T, primary *cluster.Vttablet, replicas []*cluster.Vttablet, timeToWait time.Duration, valueToInsert int) {
	...
	_, err := RunSQL(t, insertSQL, primary, cluster.DbPrefix+"ks")
	require.NoError(t, err)
	time.Sleep(100 * time.Millisecond)
...
func checkInsertedValues(t *testing.T, tablet *cluster.Vttablet, index int) error {
	selectSQL := fmt.Sprintf("select msg from "+cluster.DbPrefix+"ks.vt_insert_test where id=%d", index)
